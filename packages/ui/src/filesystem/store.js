 ...
 */
export function getFileTree(_sFileSystem) {
  return derived(_sFileSystem, ($f) => {
    const dir = {
      type: 'folder',
      name: '~',
      files: []
...
export function getFileSystem() {
  ...
  const sFileSystem = {
    ...
    /**
     * To get a list of all files which is what I’ll keep in the `sFileSystem` —
     *
     * - I’ll keep the `VFile` there but without value
     * - then use `readdir` and iterate over all strings
     *     - call `stat` on each string / path to see if it’s file or folder
     *     - if it’s folder recursively call
     *     - otherwise VFile with that particular parameters — `name`, `path`
     */
    /**
     *
     * @return {VFile[]}
